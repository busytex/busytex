...
REDEFINE_SYM := import sys; print(' '.join('-D{func}={prefix}_{func}'.format(func = func, prefix = sys.argv[1]) for func in sys.argv[2:]))
...
CFLAGS_BIBTEX := -Dmain='__attribute__((visibility(\"default\"))) busymain_bibtex8'      $(shell python3 -c $(REDEFINE_SYM) busybibtex $(BIBTEX_REDEFINE) )
CFLAGS_XDVIPDFMX := -Dmain='__attribute__((visibility(\"default\"))) busymain_xdvipdfmx' $(shell python3 -c $(REDEFINE_SYM) busydvipdfmx $(DVIPDFMX_REDEFINE) )
CFLAGS_PDFTEX := -Dmain='__attribute__((visibility(\"default\"))) busymain_pdftex'       $(shell python3 -c $(REDEFINE_SYM) busypdftex $(PDFTEX_REDEFINE) $(SYNCTEX_REDEFINE))
CFLAGS_LUATEX := -Dmain='__attribute__((visibility(\"default\"))) busymain_luatex'       $(shell python3 -c $(REDEFINE_SYM) busyluatex $(LUATEX_REDEFINE) $(SYNCTEX_REDEFINE))
...
source/texlive.patched: source/texlive.downloaded
	...
	echo $(shell python3 -c $(REDEFINE_SYM) busybibtex $(BIBTEX_REDEFINE) )
	touch $@
