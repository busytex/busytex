...
mkdir -p $DIST/$BINARCH_native && curl -o $DIST/$BINARCH_native/$BUSYTEX_native -L https://github.com/busytex/busytex/releases/download/build_native_9b40c3ce65d39b52bc38eb4794b8f9837b956064_12299351715_1/busytex && chmod +x $DIST/$BINARCH_native/$BUSYTEX_native
ln -s $TEXDIR/$BINARCH_native/$BUSYTEX_native $BUSYTEX_native
#mkdir -p $DIST/$BINARCH_native && ln -s $PWD/busytex $TEXDIR/$BINARCH_native/$BUSYTEX_native
...
for name in updmap.pl fmtutil.pl mktexlsr.pl; do  #updmap-sys.sh updmap-user.sh fmtutil-sys.sh fmtutil-user.sh; do
...
echo $(realpath $(dirname $BUSYTEX_native))
