...
class BusytexPipeline
{
    ...
    async reload_module(env, project_dir, data_packages_js = [])
    {
        ...
        const Module =
        {
            ...
            {
                WebAssembly.instantiate(wasm_module, imports).then(output => successCallback(output.instance || output)).catch(err => {throw new Error('Error while initializing BusyTex!\n\n' + err.toString())});
                return {};
            },
